...
public class JWTAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
	...
	public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException {
		...
		try {//mapper에서 읽어와서 Security에게 자격 증명을 요청하는데 필요한 member 객체를 생성함
			Member member = mapper.readValue(request.getInputStream(), Member.class);
			
			//*authToken은 name, password를 가지고 인증을 '시도'하기 위한 객체임. 성공실패아직아님
			//authenticationManager가 성공실패 결정하는것임 
			Authentication authToken = new UsernamePasswordAuthenticationToken(member.getUsername(), member.getPassword());
	        
			//디버깅 용도로 username을 로그에 기록한다.
			log.info(member.getUsername());

			//***authToken을 인증매니저한테 마침내 전달함->성공할지 실패할지는 매니저가 결정하는부분
	...
	//매니저를 거치고, 인증이 성공했을 때 실행되는 후처리 메소드 successfulAuthentication
	//역할 -> JWT 토큰을 생성하고 "response"즉 응답에 포함시킴
	@Override
	protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, Authentication authResult) throws IOException, ServletException {
		...
		String token = JWT.create()
							.withExpiresAt(new Date(System.currentTimeMillis()+1000*60*10000)) // 만료시간 설정
